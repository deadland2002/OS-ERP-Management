...
const PresentAttendanceList = (prop: Props) => {
    ...
    return (
        <>
            <div
                className={`w-full overflow-x-auto max-w-[calc(100vw-80px)] lg:max-w-[calc(100vw-250px)] minimalScroll`}>
                <div className={`flex min-w-[600px] flex-col gap-2`}>
                    <div
                        className={`flex w-full flex-wrap bg-gray-100 rounded-md px-2 py-1 justify-between font-semibold`}>
...

                    {
                        prop.data.map((single, index) => (
                            <label key={`${single.student_id}_${index}`} htmlFor={`checkbox_${index + 1}`}
                                ...
                                <input id={`checkbox_${index + 1}`}
                                       data-name={single.first_name + " " + single.last_name}
                                       data-id={single.student_id} onClick={HandleClick} className={`min-w-[20px]`}
                                       type={'checkbox'}/>
                            </span>
                            </label>
                        ))
                ...
                absentArray.size >= 1 && (
                    <div className={`flex justify-center`}>
                        <Button size={'sm'} variant={'solid'} color={'primary'} onClick={() => {
                            prop.handleMarkAbsent(absentArray)
                        }}>Mark Absent</Button>
                    </div>
                )
            }
